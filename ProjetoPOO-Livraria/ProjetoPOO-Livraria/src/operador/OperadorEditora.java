...
public class OperadorEditora {
    ...
    public Editora buscarCodigo(String codigoBuscado) {
        for (int c = 0; c < this.editoras.size(); c++) {
            if (this.editoras.get(c).getCodigoEditora().equals(codigoBuscado)) {
    ...
    public void adicionarEditoraCodigo(String codigo) {
        Scanner sc = new Scanner(System.in);
        if (buscarCodigo(codigo) != null) {
            System.out.println("\nEditora já cadastrada anteriormente!");
            return;
